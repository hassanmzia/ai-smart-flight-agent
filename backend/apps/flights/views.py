...
# Mapping of common city/region names to IATA airport codes
# Used to resolve user-friendly location inputs to codes the SERP API understands
CITY_TO_AIRPORT = {
    # US cities & regions
...
def resolve_location_to_airport_code(location: str) -> str:
    """
    Resolve a user-provided location string to an IATA airport code.

    Handles:
    - Already an IATA code (3 uppercase letters): returned as-is
    - City name with optional country/state: looked up in mapping
    - Falls back to the original input for the SERP API to handle
    """
    if not location:
        return location
    ...
    # Strip country/state suffixes and try again
    # e.g. "Sterling, Virginia, USA" → try "sterling, virginia" → "sterling"
    ...
    for i in range(len(parts)):
        ...
        if candidate in CITY_TO_AIRPORT:
            ...
            return code
        # Also try just the individual part
        if parts[i] in CITY_TO_AIRPORT:
            code = CITY_TO_AIRPORT[parts[i]]
            logger.info(f"Resolved '{location}' → {code} (component match: '{parts[i]}')")
            return code

    # Try matching just the first word (handles "Washington DC", "New York City", etc.)
    first_word = normalized.split(',')[0].strip()
    if first_word in CITY_TO_AIRPORT:
        code = CITY_TO_AIRPORT[first_word]
        logger.info(f"Resolved '{location}' → {code} (first-component match)")
        return code

    # No match found — return original for SERP API to attempt
...
def search_flights(request):
    ...
    origin_raw = request.query_params.get('origin', 'NYC')
    destination_raw = request.query_params.get('destination', 'LAX')
    ...
    # Resolve city/location names to IATA airport codes
    origin = resolve_location_to_airport_code(origin_raw)
    destination = resolve_location_to_airport_code(destination_raw)
    logger.info(f"Location resolution: '{origin_raw}' → '{origin}', '{destination_raw}' → '{destination}'")
